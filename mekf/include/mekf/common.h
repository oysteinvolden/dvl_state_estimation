...
namespace mekf{
    ...
    struct state {
        quat  quat_nominal;   // quaternion defining the rotaton from NED to body frame // TODO: opposite?
    ...
    struct dvlSample {
        vec3    vel;            // measured velocity in body frame [m/sec)] (must be converted to NED)
        bool    velocity_valid; // valid measurement
        uint64_t  time_us;      // timestamp of the measurement [uSec]
    };

    // TODO: add info on # simultaneous detections in a frame and euclidean distance?
    ...
    struct sbgNavSample {
        vec3 posNED;       // measured NED body position relative to the local origin [m]
        vec3 vel;          // measured NED velocity [m/s]
	    uint64_t time_us;  // timestamp of the measurement [micro seconds]
    };
